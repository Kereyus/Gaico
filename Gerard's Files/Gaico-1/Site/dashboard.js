...
async function loadDashboard() {
  const backlogTasksDiv = document.getElementById("backlog-tasks");
  const storiesTasksDiv = document.getElementById("stories-tasks");
  ...
  if (projects.length === 0) {
    backlogTasksDiv.innerHTML = `<p>You have no projects yet.</p>`;
    storiesTasksDiv.innerHTML = `<p>You have no projects yet.</p>`;
    todoTasksDiv.innerHTML = `<p>You have no projects yet.</p>`;
    barCanvas.replaceWith(createNoDataMessage("You have no projects. Visit the Projects page to create one."));
    pieCanvas.replaceWith(createNoDataMessage("No data to visualize. Create a project to begin tracking tasks."));
    return;
  }
  ...
  if (tasks.length === 0) {
    backlogTasksDiv.innerHTML = `<p>No tasks yet.</p>`;
    storiesTasksDiv.innerHTML = `<p>No tasks yet.</p>`;
    todoTasksDiv.innerHTML = `<p>No tasks yet.</p>`;
    barCanvas.replaceWith(createNoDataMessage("No tasks found. Add one below or through a project."));
    pieCanvas.replaceWith(createNoDataMessage("Nothing to visualize yet. Add tasks to see breakdowns."));
    return;
  }
  ...
  const storiesTasks = tasks.filter(task => task.status === "Stories");
  const todoTasks = tasks.filter(task => task.status === "To Do");

  // Function to render tasks for a given column
  const renderTasks = (tasksArray, container) => {
  ...
  renderTasks(backlogTasks, backlogTasksDiv);
  renderTasks(storiesTasks, storiesTasksDiv);
  renderTasks(todoTasks, todoTasksDiv);
...
// Task creation (same as before, but ensure the 'status' in the form matches the backend expectations)
async function createTask(e) {
  e.preventDefault();
  const form = e.target;
  const data = {
...
// Task editing (you might want to allow editing of the status as well)
function editTask(id) {
  const newTitle = prompt("Enter new task title:");
  if (!newTitle) return;
...
// If no charts, show friendly message instead (same as before)
function createNoDataMessage(text) {
  const div = document.createElement("div");
  div.textContent = text;
  div.style.textAlign = "center";
